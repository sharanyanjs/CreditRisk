...
st.subheader("📌 Portfolio Risk Distribution", help="Breakdown of predicted risk scores and loan exposure")

# Recompute risk scores
X_vis = df[['Age', 'Job', 'Credit amount', 'Duration', 'Debt_to_Income_Ratio']].copy()
X_vis[['Credit amount', 'Duration', 'Debt_to_Income_Ratio']] = scaler.transform(
    X_vis[['Credit amount', 'Duration', 'Debt_to_Income_Ratio']])
df['Risk_Score'] = model.predict_proba(X_vis)[:, 1]
...
with col1:
    st.markdown("**Distribution of Predicted Risk Scores**")
    fig1, ax1 = plt.subplots()
    sns.histplot(df['Risk_Score'], bins=20, kde=True, ax=ax1, color="#3498db")
...
with col2:
    st.markdown("**Loan Amount Distribution by Risk Category**")
    fig2, ax2 = plt.subplots()
    sns.boxplot(data=df, x='Risk_Bucket', y='Credit amount', 
                palette=["#2ecc71", "#f39c12", "#e74c3c"], ax=ax2)
    ax2.set_title("Credit Amount by Risk Bucket")
